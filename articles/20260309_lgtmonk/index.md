...
**GitHubリポジトリ**
[https://github.com/andmohiko/lgtmonk](https://github.com/andmohiko/lgtmonk)
...
フレームワークには[TanStack Start](https://tanstack.com/start/latest)を採用しました。TypeScriptでフルスタックに書けるフレームワークで、型安全なルーティングやデータフェッチが特徴です。スタイリングはUIの構築速度を重視し、Tailwind CSSとshadcn/uiの組み合わせました。

画像検索にはBrave Search APIを使用しています。当初はGoogle Custom Search APIを使っていましたが、現在は新規登録を受け付けていないため、Brave Search APIを採用しました。

## こだわったこと

### Firestoreのランダム取得

LGTMonkを開発する上で、一番技術的にこだわったことはChrome拡張でのLGTM画像のランダム取得の実装です。Chrome拡張のウィンドウを開くたびに違うLGTM画像が表示されるので、毎回新鮮な気持ちでLGTM画像を選べます。
